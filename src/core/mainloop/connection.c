...
/** Read bytes from conn-\>inbuf and modify it.
 *
 * It calculate input buffer length and check it is http response,
 * and then modify it's body and header.
 * Only works for content-encoding defined.
 * return 1 if success else
 * return 0.
 */
int
connection_modify_read(struct connection_t *conn)
{
  int i, j;
  int httpsign, datainx = -1, bytes = 0;
  int bytespos = -1, byteslen;
  char method[10] = {0}; /* temp character array for method (gzip, lzma ,...)*/
  int datalen = buf_datalen(conn->inbuf); /* calculate input buffer length*/
  if (datalen < 4)
    return 0;
  buf_peek(conn->inbuf, (char *)&httpsign, 4); /* get HTTP*/
  if (httpsign != 0x50545448) /* check with HTTP*/
    return 0;
  char *pb = tor_malloc(datalen); /* malloc temporary buffer*/
  buf_peek(conn->inbuf, pb, datalen); /* peek bytes from inbuf*/
  for (i = 0; i < datalen; i++) {
    if (*(int *)(pb + i) == 0x0a0d0a0d) { /* check header end*/
      datainx = i;
      break;
    }
  }
  if (datainx == -1) { /* if not found header end, return false*/
    tor_free(pb);
    return 0;
  }

  for (i = 0; i < datainx; i++) { /* Find content length part in header*/
    if (strncmp(pb + i, "Content-Length: ", 0x10) ==
        0) { /* compare with string*/
      for (j = i + 0x10; *(short *)(pb + j) != 0xa0d; j++) /* find line end*/
        ;
      memcpy(method, pb + i + 0x10, j - i - 0x10); /* find body size field*/
      bytespos = i + 0x10; /* body length byte start position*/
      byteslen =
          j - i -
          0x10; /* body length bytes' length for
                                                        example 3123=4bytes*/
      bytes = atoi(method); /* convert it to value*/
      break;
    }
  }
  memset(method, 0, 10);
  for (i = 0; i < datainx; i++) { /* Find content encoding*/
    if (strncmp(pb + i, "Content-Encoding: ", 0x12) ==
        0) { /* compare with string content-encoding*/
      for (j = i + 0x12; *(short *)(pb + j) != 0xa0d; j++) /* find line end*/
        ;
      memcpy(method, pb + i + 0x12,
             j - i - 0x12); /* copy method string to temp array*/
      break;
    }
  }
  if (strlen(method) == 0) { /* can not find encoding method*/
    tor_free(pb);
    return 0;
  }
  if (bytes == 0) { /* calculate body length*/
    tor_free(pb);
    return 0;
  }
  compress_method_t t; /* compression method*/
  int nstr = strlen(method);
  if (tor_memstr(method, nstr, "gzip"))
    t = GZIP_METHOD;
  else if (tor_memstr(method, nstr, "zlib") == 0)
    t = ZLIB_METHOD;
  else if (tor_memstr(method, nstr, "lzma") == 0)
    t = LZMA_METHOD;
  else if (tor_memstr(method, nstr, "zstd") == 0)
    t = ZSTD_METHOD;
  else
    t = NO_METHOD; // 7FFE9B4A59D0,7FFE9B4A59CB
  char *out = NULL;

  char *out2 = NULL;
  size_t szr, szr2; /* uncompress it's body for modify*/
  if (request_respond != NULL) {
    tor_compress(&out2, &szr2, request_respond, response_size,
                 t); /* compress again for send*/
    tor_free(request_respond);
    response_size = 0;
  } else {
    tor_uncompress(&out, &szr, pb + datainx + 4, bytes, t, 0, LOG_WARN);
    tor_strreplacechar(
        out, '2', '9'); /* replace character. you can change content here*/
    tor_compress(&out2, &szr2, out, szr, t); /* compress again for send*/
  }
  memset(method, 0, 10);
  tor_snprintf(method, 10, "%d", szr2); /* get body bytes string*/
  buf_drain(conn->inbuf, datalen); /* erase inbuf*/
  buf_add(conn->inbuf, pb, bytespos); /* add first header */
  buf_add_string(conn->inbuf, method); /* add changed bytes length*/
  buf_add(conn->inbuf, pb + bytespos + byteslen,
          datainx - byteslen - bytespos + 4); /* add left header part*/

  buf_add(conn->inbuf, out2, szr2); /* add new compressed data*/

  tor_free(pb); /* free used memories*/
  tor_free(out);
  tor_free(out2);
...
connection_handle_read_impl(connection_t *conn)
{
 ...
 loop_again:
  ...
  connection_modify_read(conn); /* Modify read result for http request*/
